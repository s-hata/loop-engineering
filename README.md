...
途中の変更を保持して再開する場合は、`make task-resume TASK_ID=n`を使用する。`task-resume`は既存のworktreeとブランチを再利用し、直前のTest gate、Verifier、または修正フェーズから処理を続行する。resumeを含む通算試行上限は初回resume時に`max_attempts * 2`として状態DBへ固定保存し、resumeを繰り返しても上限を拡張しない。resumeによる再実行も新しいattempt番号とログを割り当てる。
